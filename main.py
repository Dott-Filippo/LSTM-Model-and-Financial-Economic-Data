...
from prophet import Prophet  # Importa Prophet
...
def create_prophet_model(price_data):
    # Prepara i dati per Prophet
    df_prophet = price_data.reset_index().rename(columns={'Date': 'ds', 'Close': 'y'})
    model = Prophet()
    model.fit(df_prophet)
    
    # Genera le date future
    future = model.make_future_dataframe(periods=365)  # Previsione per un anno
...
def main():
    ...
    if st.button("Analizza"):
        with st.spinner("Elaborazione in corso... Attendere, potrebbero volerci alcuni minuti"):
            ...
            prophet_forecast = create_prophet_model(price_data)
            ...
            fig.add_trace(go.Scatter(x=prophet_forecast['ds'], y=prophet_forecast['yhat'], mode='lines', name='Previsione Prophet', line=dict(color='green')))
            fig.add_trace(go.Scatter(x=prophet_forecast['ds'], y=prophet_forecast['yhat_lower'], mode='lines', name='Intervallo Inferiore Prophet', line=dict(dash='dash', line=dict(color='lightgreen'))))
            fig.add_trace(go.Scatter(x=prophet_forecast['ds'], y=prophet_forecast['yhat_upper'], mode='lines', name='Intervallo Superiore Prophet', line=dict(dash='dash', line=dict(color='lightgreen'))))

            fig.update_layout(title=f'Previsione del Prezzo delle Azioni per {ticker} con LSTM e Prophet',
                              xaxis_title='Data',
