...
def fit_prophet_model(data_prophet, exogenous_data_prophet, lifecycle_data_prophet):
    ...
    model_prophet = Prophet(yearly_seasonality=True, daily_seasonality=False,changepoint_prior_scale=0.000000000000000000000000000000000001)
...
def main():
    ...
    logo = Image.open("Logo_Delfi.png")  # Carica l'immagine
    ...
    if st.button("Analizza"):
        with st.spinner("Elaborazione in corso... Attendere, potrebbero volerci alcuni minuti"):
            ...
            if macro_trend == True:
                fig.add_trace(go.Scatter(x=all_dates_prophet_plot, y=forecast_prophet['yhat'], mode='lines', name='Macro Trend',
